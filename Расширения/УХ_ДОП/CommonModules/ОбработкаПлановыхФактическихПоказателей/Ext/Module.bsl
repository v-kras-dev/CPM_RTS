...
//                 <продолжение описания параметра>
//
Функция ОбработатьДанные(Параметры) Экспорт  
	
	НачалоПериода = Параметры.НачалоПериода;
	КонецПериода = Параметры.КонецПериода;
	
	ТаблицаДанныхРегистра = ПолучитьДанныеРегистра();	
    ТаблицаДанныхОтчетов = ПолучитьДанныеОтчетов();
	ТаблицаРезультат = СравнитьДанныеРегистраИОтчетов(ТаблицаДанныхРегистра,ТаблицаДанныхОтчетов);
	
	ТекстСообщения = "Шаг - 4. Заполнение регистра - Значения плановых и фактических показателей";;	
	ДлительныеОперации.СообщитьПрогресс(,ТекстСообщения);
	
	НачатьТранзакцию();    
	Попытка   	
		Для Каждого СтрокаДанныхОтчета Из ТаблицаРезультат Цикл   
			ОбработатьЗаписьПлановыеФактическиеПоказатели(СтрокаДанныхОтчета);	 
		КонецЦикла;     
	...
	РезультатВыборкаСверка = Новый Структура("ИмяКоманды, КоличествоЗаписей", "ОбновлениеДанныхПланФакт", ТаблицаРезультат.Количество());
...
Функция ПолучитьДанныеРегистра()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
		|	ЗначенияПлановыхФактическихПоказателей.Организация КАК Организация,
		|	ЗначенияПлановыхФактическихПоказателей.ПериодОтчета КАК ПериодОтчета,
		|	ЗначенияПлановыхФактическихПоказателей.Проект КАК Проект,
		|	ЗначенияПлановыхФактическихПоказателей.СтатьяЗатрат КАК СтатьяЗатрат,
		|	ЗначенияПлановыхФактическихПоказателей.СтатьяДоходовРасходов КАК СтатьяДоходовРасходов,
		|	ЗначенияПлановыхФактическихПоказателей.НазначениеПлатежа КАК НазначениеПлатежа,
		|	ЗначенияПлановыхФактическихПоказателей.Контрагент КАК Контрагент,
		|	ЗначенияПлановыхФактическихПоказателей.Регион КАК Регион,
		|	ЗначенияПлановыхФактическихПоказателей.ЗначениеПлан КАК ЗначениеПлан,
		|	ЗначенияПлановыхФактическихПоказателей.ЗначениеФакт КАК ЗначениеФакт,
		|	ЗначенияПлановыхФактическихПоказателей.ЗначениеОтклонение КАК ЗначениеОтклонение,
		|	ЗначенияПлановыхФактическихПоказателей.ЗначениеОтклонениеПроцент КАК ЗначениеОтклонениеПроцент
		|ИЗ
		|	РегистрСведений.ЗначенияПлановыхФактическихПоказателей КАК ЗначенияПлановыхФактическихПоказателей
		|ГДЕ
		|	ЗначенияПлановыхФактическихПоказателей.ПериодОтчета.ДатаНачала МЕЖДУ &ДатаНачала И &ДатаОкончания";
		
    Возврат Запрос.Выполнить().Выгрузить();

КонецФункции // ПолучитьДанныеРегистра()      
#Область ПолучениеДанныхПланФакт
// <Описание функции>
//
// Параметры:
//  <Параметр1>  - <Тип.Вид> - <описание параметра>
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция ПолучитьДанныеОтчетов() 
	ДанныеЭО = ПолучениеДанныхЭкземпляровОтчета();
    ТаблицаДанныхОтчета = СведемПланФакт(ДанныеЭО);
	Возврат ТаблицаДанныхОтчета;	

КонецФункции // ПолучитьДанныеОтчетов() 
// <Описание функции>
//
// Параметры:
//  <Параметр1>  - <Тип.Вид> - <описание параметра>
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция ПолучениеДанныхЭкземпляровОтчета()  	
	
	ТекстСообщения = "Шаг - 1. Получаем данные план";;	
	ДлительныеОперации.СообщитьПрогресс(,ТекстСообщения);   
	...
	НомерСтроки = 0;   
	МаксКол = РезультатЗапроса.Количество();
	Для Каждого Стр Из РезультатЗапроса Цикл    
		...
		//ПроцентВыполнения = (НомерСтроки/МаксКол)*100;
	...
	ОбработаемОтдельноОтчетыИзГруппы_ИТРазработкаИСопровождениеПО2(ТаблицаДанных,ТаблицаДанных_Косвенные);	
...
//
//
Процедура ОбработаемОтдельноОтчетыИзГруппы_ИТРазработкаИСопровождениеПО2(ТаблицаДанных,ТаблицаДанных_Косвенные)
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция СведемПланФакт(ТаблицаДанных_План)
	ТаблицаДанных_Факт = ПолучениеДанныхФакта();	
	ТаблицаДанныхОтчета = СопоставлениеДанныхПланФакт(ТаблицаДанных_План,ТаблицаДанных_Факт);	
	Возврат ТаблицаДанныхОтчета;
...
Функция ПолучениеДанныхФакта()
	ТекстСообщения = "Шаг - 2. Получаем данные факт";;	
	ДлительныеОперации.СообщитьПрогресс(,ТекстСообщения);
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция СопоставлениеДанныхПланФакт(ТаблицаДанных_План,ТаблицаДанных_Факт)  
	
	ТекстСообщения = "Шаг - 3. Сопоставим данные план - факт";;	
	ДлительныеОперации.СообщитьПрогресс(,ТекстСообщения);
	...
	РезультатЗапроса.Колонки.Добавить("УдалитьЗапись", ОписаниеТипаБулево);
	РезультатЗапроса.Колонки.Добавить("ДобавитьЗапись", ОписаниеТипаБулево);
	РезультатЗапроса.Колонки.Добавить("ОбновитьЗапись", ОписаниеТипаБулево);
	...
	Для Каждого Стр Из РезультатЗапроса Цикл    		
		...
		ИначеЕсли Стр.Сумма = NULL И Стр.СуммаФакт <> NULL  Тогда   
			...
			Стр.ОтклонениеПроцент = 100;
		КонецЕсли;	
		Стр.УдалитьЗапись = Ложь;
		Стр.ДобавитьЗапись = Истина;
		Стр.ОбновитьЗапись = Ложь;
	КонецЦикла;
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция ОчиститьЗаписиРегистраЗаВыбранныйПериод(Параметры) Экспорт   
	
	НаборЗаписей = РегистрыСведений.ЗначенияПлановыхФактическихПоказателей.СоздатьНаборЗаписей();
	...
	//Запрос.Текст = 
	//	"ВЫБРАТЬ
	//	|	ЗначенияПлановыхФактическихПоказателей.Затраты КАК Затраты,
	//	|	ЗначенияПлановыхФактическихПоказателей.Организация КАК Организация,
	//	|	ЗначенияПлановыхФактическихПоказателей.ПериодОтчета КАК ПериодОтчета,
	//	|	ЗначенияПлановыхФактическихПоказателей.Проект КАК Проект,
	//	|	ЗначенияПлановыхФактическихПоказателей.СтатьяЗатрат КАК СтатьяЗатрат,
	...
	//	|	ЗначенияПлановыхФактическихПоказателей.ЗначениеОтклонение КАК ЗначениеОтклонение,
	//	|	ЗначенияПлановыхФактическихПоказателей.ЗначениеОтклонениеПроцент КАК ЗначениеОтклонениеПроцент,
	//	|	ЗначенияПлановыхФактическихПоказателей.УникальныйИдентификаторЗаписи КАК УникальныйИдентификаторЗаписи
	//	|ИЗ
	//	|	РегистрСведений.ЗначенияПлановыхФактическихПоказателей КАК ЗначенияПлановыхФактическихПоказателей
	//	|ГДЕ
	//	|	ЗначенияПлановыхФактическихПоказателей.ПериодОтчета.ДатаНачала МЕЖДУ &НачалоПериода И &КонецПериода";    
	//
	...
	//	УдалитьЗаписьПлановыеФактическиеПоказатели(ВыборкаДетальныеЗаписи)	 	
	//КонецЦикла;
	
КонецФункции // ПолноеУдалениеЗаписейРегистраСведений()
// <Описание функции>
//
// Параметры:
//  <Параметр1>  - <Тип.Вид> - <описание параметра>
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция СравнитьДанныеРегистраИОтчетов(ТаблицаДанныхРегистра,ТаблицаДанныхОтчетов)  
	
	//Если регистр заполняется впервые, заполним его данными из отчетов
	Если ТаблицаДанныхРегистра.Количество()  = 0 Тогда    
		Возврат ТаблицаДанныхОтчетов;
	Иначе  
			
	КонецЕсли;
	
КонецФункции // СравнитьДанныеРегистраИОтчетов()  
// <Описание процедуры>
//
// Параметры:
//  <Параметр1>  - <Тип.Вид> - <описание параметра>
...
//                 <продолжение описания параметра>
//
Процедура ОбработатьЗаписьПлановыеФактическиеПоказатели(СтрокаДанныхОтчета)

	Если СтрокаДанныхОтчета.УдалитьЗапись Тогда  
		УдалитьЗаписьПлановыеФактическиеПоказатели(СтрокаДанныхОтчета);				
	ИначеЕсли СтрокаДанныхОтчета.ДобавитьЗапись Тогда 
		СоздатьЗаписьПлановыеФактическиеПоказатели(СтрокаДанныхОтчета)	
	ИначеЕсли СтрокаДанныхОтчета.ОбновитьЗапись Тогда 
		ОбновитьЗаписьПлановыеФактическиеПоказатели(СтрокаДанныхОтчета)		
	КонецЕсли;

КонецПроцедуры // ОбработатьЗаписьПлановыеФактическиеПоказатели()
// <Создает запись в регистре сведений "ЗначенияПлановыхФактическихПоказателей" на основе данных отчета>
//
// Параметры:
//  <СтрокаДанныхОтчета - <Структура> - <Данные для записи в регистр, должна содержать поля:>  
//                      * <Организация>
//                 		* <ПериодОтчета>	 
//                      * <Проект>
//                      * <СтатьяЗатрат>
...
//                 		* <Регион>	 
//
Процедура СоздатьЗаписьПлановыеФактическиеПоказатели(СтрокаДанныхОтчета)

	МенеджерЗаписи = РегистрыСведений.ЗначенияПлановыхФактическихПоказателей.СоздатьМенеджерЗаписи();    
	МенеджерЗаписи.УникальныйИдентификаторЗаписи = Новый УникальныйИдентификатор;
	МенеджерЗаписи.Затраты = СтрокаДанныхОтчета.Направление;  
	МенеджерЗаписи.Организация = СтрокаДанныхОтчета.Организация;
	МенеджерЗаписи.ПериодОтчета = СтрокаДанныхОтчета.ПериодОтчета;
	МенеджерЗаписи.Проект = СтрокаДанныхОтчета.Проект;
	МенеджерЗаписи.СтатьяЗатрат = СтрокаДанныхОтчета.СтатьяЗатрат;
	МенеджерЗаписи.СтатьяДоходовРасходов = СтрокаДанныхОтчета.СтатьяДоходовРасходов;
	МенеджерЗаписи.НазначениеПлатежа = СтрокаДанныхОтчета.НазначениеПлатежа;
	МенеджерЗаписи.Контрагент = СтрокаДанныхОтчета.Контрагент;
	МенеджерЗаписи.Регион = СтрокаДанныхОтчета.Регион;
	МенеджерЗаписи.ЗначениеПлан = СтрокаДанныхОтчета.Сумма;
	МенеджерЗаписи.ЗначениеФакт = СтрокаДанныхОтчета.СуммаФакт;
	МенеджерЗаписи.ЗначениеОтклонение = СтрокаДанныхОтчета.СуммаОтклонение;  
	МенеджерЗаписи.ЗначениеОтклонениеПроцент = СтрокаДанныхОтчета.ОтклонениеПроцент;
	МенеджерЗаписи.Квартал = СтрокаДанныхОтчета.ПериодОтчета.Родитель;
	//ЗаполнитьЗначенияСвойств(МенеджерЗаписи,СтрокаДанныхОтчета);
	МенеджерЗаписи.Записать();
	
КонецПроцедуры // СоздатьЗаписьПлановыеФактическиеПоказатели()   
// <Удаляет запись в регистре сведений "ЗначенияПлановыхФактическихПоказателей" на основе данных отчета>
//
// Параметры:
//  <Параметр1>  - <Тип.Вид> - <описание параметра>
//                 <продолжение описания параметра>
...
//                 <продолжение описания параметра>
//
Процедура УдалитьЗаписьПлановыеФактическиеПоказатели(СтрокаДанныхОтчета)

	ЗаписьРегистра = РегистрыСведений.ЗначенияПлановыхФактическихПоказателей.СоздатьНаборЗаписей();
	ЗаписьРегистра.Отбор.УникальныйИдентификаторЗаписи.Установить(СтрокаДанныхОтчета.УникальныйИдентификаторЗаписи);
	ЗаписьРегистра.Записать();

КонецПроцедуры // УдалитьЗаписьПлановыеФактическиеПоказатели()
// <Обновляет запись в регистре сведений "ЗначенияПлановыхФактическихПоказателей" на основе данных отчета>
//
// Параметры:
//  <Параметр1>  - <Тип.Вид> - <описание параметра>
//                 <продолжение описания параметра>
...
//                 <продолжение описания параметра>
//
Процедура ОбновитьЗаписьПлановыеФактическиеПоказатели(СтрокаДанныхОтчета)

	ЗаписьРегистра = РегистрыСведений.ЗначенияПлановыхФактическихПоказателей.СоздатьНаборЗаписей();
	ЗаписьРегистра.Отбор.УникальныйИдентификаторЗаписи.Установить(СтрокаДанныхОтчета.УникальныйИдентификаторЗаписи);
	ЗаписьРегистра.Прочитать();
	Если ЗаписьРегистра.Количество() Тогда    
		ЗаполнитьЗначенияСвойств(ЗаписьРегистра,СтрокаДанныхОтчета);
		ЗаписьРегистра.Записать();
	КонецЕсли;
	
КонецПроцедуры // ОбновитьЗаписьПлановыеФактическиеПоказатели()
#КонецОбласти
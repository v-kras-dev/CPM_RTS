		...
		Если Стр.Аналитика1 = Справочники.ПроизвольныйКлассификаторУХ.ПустаяСсылка() Тогда	
			Если МассивДанныхРаспределения.Количество() > 0 Тогда   
				Для Каждого Эл Из МассивДанныхРаспределения Цикл  
					...
					СтруктураПараметров = Новый Структура("АналитикаПроектНаименование,АналитикаПроект,ПроцентБЛ,ПризнакПереводаПроекта,ПроектПеревода_Косвенные,Организация,СтатьяЗатрат_Мотивация,СтатьяЗатрат_ФОТНалоги,SalesTech_Проект,Поставщики_Проект,ТаблицаДанных_Косвенные,ДляПлана",
...
Функция ПолучениеСоответсвияПорядкаСортировкиНаправление()	
	...
	СоответствиеПорядкаСортировкиНаправление.Вставить(Справочники.ПроизвольныйКлассификаторУХ.НайтиПоКоду("000000019",,,ВладелецСправочника),13); //НСИ/ST
	СоответствиеПорядкаСортировкиНаправление.Вставить(Справочники.ПроизвольныйКлассификаторУХ.НайтиПоКоду("000000022",,,ВладелецСправочника),14); //ОЛ
	СоответствиеПорядкаСортировкиНаправление.Вставить(Справочники.ПроизвольныйКлассификаторУХ.НайтиПоКоду("000000005",,,ВладелецСправочника),15); //Капитал
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция РассчитаемКосвенныеРасходыОтдельноФакт(ТаблицаДанных_Косвенные,ТаблицаПроцентов_РаспределениеКосвенные,СоответствияПорядкаСортировкиСтатьиЗатрат,ПризнакРасчетаПланФакт)
	...
	//ТаблицаРасчета.Свернуть("Организация,Направление,Проект,СтатьяЗатрат,ПериодОтчета");  
		...
		Для Каждого ЭлПроект Из ДанныеПоПроектам Цикл
			...
			//Для Каждого СтрЗтр Из ТаблицаЗатрат Цикл
				...
				Для Каждого СтрПоСотр Из ДанныеПоСотруднику Цикл 
					...
					//НовСтр.НазначениеПлатежа = СтрПоСотр.НазначениеПлатежа; 
					//НовСтр.Контрагент = СтрПоСотр.Контрагент;
					//НовСтр.Регион = СтрПоСотр.Регион;
...
Функция СопоставлениеДанныхПланФакт(ТаблицаДанных_План,ТаблицаДанных_Факт,ДатаНачалаОтчета,ДатаОкончанияОтчета)
	...
	//{Воронков А.Д. 19.05.2025 ++
	//Задача:https://tfs.rts-tender.ru/tfs/DefaultCollection/RTS.1C/_workitems/edit/409720/
	РезультатЗапроса = ОтсортируемТаблицуДанныхПередВыводом(РезультатЗапроса);	
	//--}	
...
Функция ПолучениеЗапросаВыборкаДанныхФакта()     
	...
	"ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ВТ_НечисловыеПоказатели_ПроектПеревод
		|ИЗ
		|	РегистрСведений.ЗначенияПоказателейОтчетовНечисловые КАК ЗначенияПоказателейОтчетовНечисловые
		|ГДЕ
		...
		|	И ЗначенияПоказателейОтчетовНечисловые.ПравилоОбработки.Владелец.Родитель.Код В(&МассивПравилОбработки)
		|	И ЗначенияПоказателейОтчетовНечисловые.Версия.Сценарий В(&Сценарий) 
		|   И ЗначенияПоказателейОтчетовНечисловые.Значение <> НЕОПРЕДЕЛЕНО 
		|   И ЗначенияПоказателейОтчетовНечисловые.Значение <> """" 
		...
		|	ВТ_ЧисловыеПоказатели.ПериодЗаписи КАК ПериодЗаписи,
		|	ВТ_НечисловыеПоказатели.Значение КАК Значение,
		|	ВТ_НечисловыеПоказатели_ПроектПеревод.Значение КАК ПереводПроект
		|ИЗ
		|	ВТ_ЧисловыеПоказатели КАК ВТ_ЧисловыеПоказатели
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_НечисловыеПоказатели КАК ВТ_НечисловыеПоказатели
		|		ПО (ВТ_ЧисловыеПоказатели.Проект = ВТ_НечисловыеПоказатели.Проект)
		...
		|			И (ВТ_ЧисловыеПоказатели.Контрагент = ВТ_НечисловыеПоказатели.Контрагент)
		|			И (ВТ_ЧисловыеПоказатели.Регион = ВТ_НечисловыеПоказатели.Регион)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_НечисловыеПоказатели_ПроектПеревод КАК ВТ_НечисловыеПоказатели_ПроектПеревод
		|		ПО (ВТ_ЧисловыеПоказатели.Проект = ВТ_НечисловыеПоказатели_ПроектПеревод.Проект)
		|			И (ВТ_ЧисловыеПоказатели.СтатьяЗатрат = ВТ_НечисловыеПоказатели_ПроектПеревод.СтатьяЗатрат)
		|			И (ВТ_ЧисловыеПоказатели.СтатьяДоходовРасходов = ВТ_НечисловыеПоказатели_ПроектПеревод.СтатьяДоходовРасходов)
		|			И (ВТ_ЧисловыеПоказатели.НазначениеПлатежа = ВТ_НечисловыеПоказатели_ПроектПеревод.НазначениеПлатежа)
		|			И (ВТ_ЧисловыеПоказатели.Контрагент = ВТ_НечисловыеПоказатели_ПроектПеревод.Контрагент)
		|			И (ВТ_ЧисловыеПоказатели.Регион = ВТ_НечисловыеПоказатели_ПроектПеревод.Регион)  
		|			И (ВТ_ЧисловыеПоказатели.Организация = ВТ_НечисловыеПоказатели_ПроектПеревод.Организация)
		|			И (ВТ_ЧисловыеПоказатели.ВерсияСценарий = ВТ_НечисловыеПоказатели_ПроектПеревод.ВерсияСценарий)
		|ГДЕ
		|	ВТ_НечисловыеПоказатели.Значение ЕСТЬ NULL
		|                //ВТ_ЧисловыеПоказатели.Контрагент В( &Контрагент)";
...
Процедура ОбработаемКосвенныеРасходыОтдельно(РезультатЗапроса,ТаблицаДанных_Косвенные,ПризнакРасчетаПланФакт,ПризнакПолученияПроцентов)
	...
	ДанныеПоКосвеннымРасходам = РассчитаемКосвенныеРасходыОтдельноФакт(ТаблицаДанных_Косвенные,ТаблицаПроцентов_РаспределениеКосвенные,СоответствияПорядкаСортировкиСтатьиЗатрат,ПризнакРасчетаПланФакт);
...
//                     5. Сумма (по убыванию)
Функция ОтсортируемТаблицуДанныхПередВыводом(РезультатЗапроса)

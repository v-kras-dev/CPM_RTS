...
Функция ПолучениеСоответсвияПорядкаСортировкиНаправление()	
	...
	СоответствиеПорядкаСортировкиНаправление.Вставить("OPEX",3); 
	...
	Для Каждого СтрРасч Из ТаблицаРасчета Цикл         
		...
		Для Каждого ЭлПроект Из ДанныеПоПроектам Цикл
			...
			Для Каждого СтрПоСотр Из ДанныеПоСотруднику Цикл 
				СтруктураОтбораПроцентов = Новый Структура("Организация,Проект,ПериодОтчета",СтрПоСотр.Организация,ЭлПроект.Проект,СтрПоСотр.ПериодОтчета);	
...
Функция СопоставлениеДанныхПланФакт(ТаблицаДанных_План,ТаблицаДанных_Факт,ДатаНачалаОтчета,ДатаОкончанияОтчета)
	...
	//РезультатЗапроса.Сортировать("СортировкаНаправление");
